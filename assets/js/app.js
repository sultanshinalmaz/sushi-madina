...
(function () {
  ...
  function initIntro() {
    ...
    function close() {
      ...
      try { sessionStorage.setItem('sushi-intro', '1'); } catch (e) {}
    }
    ...
    function run() {
      ...
      var MIN_SHOW = 3600;  // минимум на экране, чтобы анимация доиграла целиком
  ...
  function renderCart() {
    ...
    if (!count) {
      list.innerHTML = '<div class="cart__empty">' + ICONS.cart +
        ...
        '<button class="btn btn--gold" data-close-cart>Открыть меню</button></div>';
      return;
    }
  ...
  function initCart() {
    ...
    document.addEventListener('click', function (e) {
      ...
      if (order) {
        var who = order.dataset.order;
        window.open(waLink(who, cartCount() ? orderText() : null), '_blank', 'noopener');
      }
    });
    ...
    if (more) {
      ...
      if (hidden.length) {
        ...
        more.addEventListener('click', function (e) {
          ...
          if (!isCollapsed) $('#reviews').scrollIntoView({ behavior: 'smooth', block: 'start' });
        });
      }
    }
